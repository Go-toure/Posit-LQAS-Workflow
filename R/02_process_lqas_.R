...
# ============================================================
# Smart FM_Child Harmonizer
# Only applies FM_ChildR / FM_ChildL where they exist
# ============================================================

update_fm_child_dynamic <- function(df, file_name = NA_character_) {
  idx_from_any <- names(df) |>
    stringr::str_match("^Count_HH\\[(\\d+)\\]/(FM_Child|FM_ChildR|FM_ChildL)$") |>
    (\(m) m[, 2])() |>
    na.omit() |>
    unique() |>
    as.integer() |>
  ...
  if (length(idx_from_any) == 0) {
    stop(paste0("No FM child columns found in file: ", file_name))
  }
  ...
  for (ii in idx_from_any) {
    ...
    has_fm <- col_fm %in% names(df)
    has_r  <- col_r %in% names(df)
    has_l  <- col_l %in% names(df)
    
    # Standard case: FM_Child exists, no R/L -> leave as is
    if (has_fm && !has_r && !has_l) next
    
    # R/L case: build or update FM_Child
    if (has_r || has_l) {
      expr_r <- if (has_r) rlang::sym(col_r) else rlang::expr(0)
      expr_l <- if (has_l) rlang::sym(col_l) else rlang::expr(0)
      
      if (has_fm) {
        mutate_list[[col_fm]] <- rlang::expr(
          ifelse(
            dplyr::coalesce(!!expr_r, 0) + dplyr::coalesce(!!expr_l, 0) >= 1 |
              dplyr::coalesce(!!rlang::sym(col_fm), 0) == 1,
            1, 0
          )
        )
      } else {
  ...
  if (length(mutate_list) == 0) return(df)
  df %>% dplyr::mutate(!!!mutate_list)
}

safe_total_missed <- function(total_sampled, missed_child) {
  dplyr::case_when(
    is.na(total_sampled) ~ NA_real_,
    total_sampled == 0 & missed_child == 0 ~ NA_real_,
    total_sampled < 60 ~ (60 - total_sampled) + missed_child,
    TRUE ~ missed_child
  )
}
...
process_regular_file <- function(file_path, file_name) {
  ...
  standardize_informed_sia <- function(x) {
    ...
    case_when(
      x %in% c("Y", "1", "Yes", "YES", "yes") ~ 1,
      x %in% c("N", "0", "No", "NO", "no") ~ 0,
      TRUE ~ NA_real_
    )
  }
  ...
  data <- update_fm_child_dynamic(data, file_name = file_name)
  ...
  if (length(sex_cols) == 0) {
    stop(paste0("No Sex_Child columns detected in file: ", file_name))
  }
  if (length(fm_cols) == 0) {
    stop(paste0("No FM_Child columns detected after harmonization in file: ", file_name))
  }
  ...
  data <- data %>%
    mutate(
      Count_HH_count = as.character(Count_HH_count),
      Count_HH_count = stringr::str_squish(Count_HH_count),
      Count_HH_count = dplyr::na_if(Count_HH_count, ""),
      Count_HH_count = dplyr::na_if(Count_HH_count, "."),
      Count_HH_count = dplyr::na_if(Count_HH_count, "NA"),
      Count_HH_count = dplyr::na_if(Count_HH_count, "null"),
      Count_HH_count = dplyr::na_if(Count_HH_count, "NULL"),
      Count_HH_count = dplyr::na_if(Count_HH_count, "-"),
      Count_HH_count = dplyr::na_if(Count_HH_count, "--"),
      Count_HH_count = as.numeric(Count_HH_count)
    ) %>%
    mutate(across(
      all_of(setdiff(numeric_count_cols, "Count_HH_count")),
  ...
  F2 <- F1 %>%
    ...
    mutate(
      ...
      total_missed = safe_total_missed(total_sampled, missed_child)
    ) %>%
    filter(cluster >= 3) %>%
    mutate(
  ...
  F4 <- F3 %>%
    ...
    mutate(
      ...
      across(c(r_Non_Compliance, r_House_not_visited, r_childabsent,
             ...
             ~ ifelse(total_missed == 0 | is.na(total_missed), 0, (.x / total_missed) * 100),
             .names = "prct_{.col}"),
