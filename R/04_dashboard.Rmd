...
output: 
  html_document:
    ...
    code_folding: show
params:
  data_file: "data/final/lqas_final.parquet"
  run_date: !r Sys.Date()
---
...
if (file.exists(params$data_file)) {
  dt <- read_parquet(params$data_file)
  data_available <- TRUE
} else {
  dt <- data.table()
...
if ("total_vaccinated" %in% names(dt) && "total_sampled" %in% names(dt)) {
  ...
  ggplot(country_summary, aes(x = reorder(country, coverage), y = coverage, fill = coverage)) +
    geom_bar(stat = "identity") +
    coord_flip() +
    labs(title = "Vaccination Coverage by Country", x = "Country", y = "Coverage (%)") +
    theme_minimal() +
    scale_fill_gradient(low = "red", high = "green", limits = c(0, 100))
}
datatable(
  dt[, .(country, province, district, response, roundNumber, total_sampled, total_vaccinated)][1:100],
  options = list(pageLength = 10, scrollX = TRUE),
  caption = "Data Sample (first 100 rows)"
)
...
# List final R files
ls -la R/
cat > R/special_272.R << 'EOF'
#!/usr/bin/env Rscript
# ============================================================
# Special Case: Process 272.rds (Nigeria LQAS Data)
# GitHub Actions Compatible Version - Parquet Format
# ============================================================
...
log_info("Starting Special Case 272 Processing (Parquet Format)")
log_info("=" %>% paste(rep("=", 60), collapse = ""))

# Parse arguments
parser <- ArgumentParser()
parser$add_argument("--input-file", default = "data/raw/272.parquet", 
                   help = "Input Parquet file path")
parser$add_argument("--lookup-file", default = "data/lookup/lqas_lookup.xlsx", 
                   help = "Lookup Excel file path")
parser$add_argument("--output-file", default = "data/processed/272.csv", 
...
process_272_data <- function(input_file, lookup_file, output_file) {
  ...
  if (!file.exists(input_file)) {
    ...
    log_info("Trying to read from CSV if available...")
    
    # Try CSV fallback
    csv_file <- gsub("\\.parquet$", ".csv", input_file)
    if (file.exists(csv_file)) {
      log_info("Found CSV fallback: {csv_file}")
      df <- fread(csv_file)
    } else {
      log_error("No input file found")
      return(NULL)
    }
  } else {
    # Read Parquet file
    df <- as.data.table(read_parquet(input_file))
  }
  ...
  for (col in char_cols) {
    set(df, j = col, value = iconv(df[[col]], from = "", to = "UTF-8", sub = ""))
  }
  
  # Standardize column names if needed
  if (!("states" %in% names(df))) {
    if ("state" %in% names(df)) df[, states := state]
    if ("State" %in% names(df)) df[, states := State]
    if ("STATE" %in% names(df)) df[, states := STATE]
  }
  
  if (!("lgas" %in% names(df))) {
    if ("lga" %in% names(df)) df[, lgas := lga]
    if ("LGA" %in% names(df)) df[, lgas := LGA]
    if ("LGAS" %in% names(df)) df[, lgas := LGAS]
  }
  
  if (!("today" %in% names(df))) {
    if ("Today" %in% names(df)) df[, today := Today]
    if ("date" %in% names(df)) df[, today := date]
    if ("Date" %in% names(df)) df[, today := Date]
    if ("submission_time" %in% names(df)) df[, today := submission_time]
  }
  ...
  log_info("✅ Successfully processed 272: {nrow(df)} rows")
