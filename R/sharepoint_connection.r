...
#
# SUPERSEDED by R/06_push_to_sharepoint.R -- kept here unused, not
# deleted, in case anything still refers to it by name.
#
# Two problems with this file that 06_push_to_sharepoint.R fixes:
#   1) SECURITY: this file used to have a live SharePoint app
#      registration's tenant_id / client_id / client_secret hardcoded
#      in plain text below -- and this file is tracked in git, with
#      that secret sitting in this repo's commit history. Those
#      values have been removed from this file; if they were ever
#      pushed to the GitHub remote, treat that app registration's
#      client secret as compromised and rotate it in Azure AD app
#      registrations, then update config/secrets.env (git-ignored)
#      with the new value. Credentials now load from
#      config/secrets.env via R/utils/env_loader.R, same as
#      06_push_to_sharepoint.R and the rest of this WHO AFRO tooling.
#   2) WRONG TARGET: "Method 1" below uploads to the ROOT of the
#      site's default document library, not to the "7. SIA_Data/Data
#      Repository" folder the file actually needs to land in --
#      because it resolves the site as just "sites/AF-pep" and treats
#      "GISWORKSPACE" as if it were a folder, when it's actually part
#      of the site's own address (sites/AF-pep/GISWORKSPACE). "Method
#      2" (interactive login) does target the right folder, but
#      requires a person to click through a WHO SSO login every run,
#      so it can't run unattended from a scheduled task.
#
# Left in place for reference / manual troubleshooting only.
# ============================================================
...
# SharePoint configuration -- from config/secrets.env, see
# config/secrets.env.example. Never hardcode these.
...
# ============================================================
# MAIN EXECUTION -- disabled by default now that this file is
# superseded. Uncomment to use it manually for troubleshooting.
# ============================================================

cat("This script is superseded by R/06_push_to_sharepoint.R and no longer runs\n")
cat("automatically. See the header comment for why, and for the credential\n")
cat("rotation note if this file's old hardcoded secret was ever pushed to git.\n")
