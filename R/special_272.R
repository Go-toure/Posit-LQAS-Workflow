#!/usr/bin/env Rscript
# ============================================================
# Special Case: Process 272.rds (Nigeria LQAS Data)
# GitHub Actions Compatible Version
# Input: data/input/272.rds
# Output: data/output/272.csv
# ============================================================
...
# Configure logging
log_appender(appender_file("logs/special_272.log"))
log_info("=" %>% paste(rep("=", 60), collapse = ""))
log_info("Starting Special Case 272 Processing")
log_info("=" %>% paste(rep("=", 60), collapse = ""))

# Parse arguments
parser <- ArgumentParser()
parser$add_argument("--input-file", default = "data/input/272.rds", 
                    help = "Input RDS file path")
parser$add_argument("--lookup-file", default = "data/lookup/lqas_lookup.xlsx", 
                    help = "Lookup Excel file path")
parser$add_argument("--output-file", default = "data/output/272.csv", 
...
process_272_data <- function(input_file, lookup_file, output_file) {
  ...
  if (!("states" %in% names(df))) {
    if ("state" %in% names(df)) df$states <- df$state
    if ("State" %in% names(df)) df$states <- df$State
    if ("STATE" %in% names(df)) df$states <- df$STATE
  }
  
  if (!("lgas" %in% names(df))) {
    if ("lga" %in% names(df)) df$lgas <- df$lga
    if ("LGA" %in% names(df)) df$lgas <- df$LGA
    if ("LGAS" %in% names(df)) df$lgas <- df$LGAS
  }
  
  if (!("today" %in% names(df))) {
    if ("Today" %in% names(df)) df$today <- df$Today
    if ("date" %in% names(df)) df$today <- df$date
    if ("Date" %in% names(df)) df$today <- df$Date
    if ("submission_time" %in% names(df)) df$today <- df$submission_time
  }
  
  if (!("Cluster" %in% names(df))) df$Cluster <- NA
  ...
  log_info("✅ Successfully processed 272.rds: {nrow(df)} rows")
...
result <- process_272_data(args$input_file, args$lookup_file, args$output_file)
