...
# COMPLETE VERSION - Adapted from original script
# Reads Parquet files (from Python fetcher)
# Keeps ALL original district/province mappings
# ============================================================
...
parser$add_argument("--input-dir", default = "data/processed", 
                    help = "Input directory with processed Parquet files")
parser$add_argument("--lookup-file", default = "data/lookup/lqas_lookup.xlsx", 
                    help = "Lookup Excel file path for date harmonization")
parser$add_argument("--output-file", default = "data/final/lqas_final.parquet", 
...
# ============================================================
# STEP 1: READ PROCESSED PARQUET FILES (instead of CSV)
# ============================================================
log_info("\n📁 STEP 1: Reading processed Parquet files from {args$input_dir}")
...
if (length(parquet_files) == 0) {
  ...
  log_info("Trying to read CSV files as fallback...")
  csv_files <- list.files(args$input_dir, pattern = "\\.csv$", full.names = TRUE)
  if (length(csv_files) > 0) {
    log_info("Found {length(csv_files)} CSV files")
    combined_data <- data.table()
    for (file in csv_files) {
      dt <- fread(file)
      combined_data <- rbindlist(list(combined_data, dt), fill = TRUE)
    }
    G <- as_tibble(combined_data)
  } else {
    quit(status = 1)
  }
} else {
...
log_info("Loaded {nrow(G)} rows, {ncol(G)} columns")

# ============================================================
# STEP 2: BASIC DATA CLEANING (from original script)
# ============================================================
log_info("\n🧹 STEP 2: Basic data cleaning")

C <- G %>%
  filter(start_date > as_date(args$start_date)) %>%
  arrange(start_date)

log_info("After date filter: {nrow(C)} rows")
...
# ============================================================
# STEP 3: COUNTRY NAME STANDARDIZATION (from original script)
# ============================================================
...
# ============================================================
# STEP 4: DISTRICT NAME CORRECTIONS (FULL MAPPINGS FROM ORIGINAL)
# ============================================================
log_info("\n🏙️ STEP 4: Applying district name corrections (FULL MAPPINGS)")
...
# ============================================================
# STEP 5: PROVINCE NAME CORRECTIONS (FULL MAPPINGS FROM ORIGINAL)
# ============================================================
log_info("\n🗺️ STEP 5: Applying province name corrections (FULL MAPPINGS)")
...
# ============================================================
# STEP 6: AFRO BLOCK CLASSIFICATION (from original script)
# ============================================================
log_info("\n🌍 STEP 6: Adding AFRO block classification")
...
G <- G %>%
  mutate(
    AFRO_block = case_when(
      ...
      country %in% c("RWANDA", "BURUNDI", "ANGOLA", "KENYA", "ERITREA", "ETHIOPIA", 
...
# ============================================================
# STEP 7: DATE HARMONIZATION WITH LOOKUP TABLE
# ============================================================
...
if (!file.exists(args$lookup_file)) {
  ...
  log_info("Read lookup file from {args$lookup_file}")
}
...
# ============================================================
# STEP 8: FINAL TRANSFORMATIONS (from original script)
# ============================================================
...
log_info("Final dataset: {nrow(G)} rows, {ncol(G)} columns")

# ============================================================
# STEP 9: SAVE FINAL OUTPUT
# ============================================================
log_info("\n💾 STEP 9: Saving final output")

# Create output directory if needed
dir_create(dirname(args$output_file))

# Save as Parquet
write_parquet(G, args$output_file)
log_info("✅ Saved final data to {args$output_file}")

# Also save as CSV for compatibility
csv_output <- gsub("\\.parquet$", ".csv", args$output_file)
fwrite(G, csv_output)
log_info("✅ Also saved to {csv_output}")
